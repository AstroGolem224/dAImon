...
| `ears` | `SystemCallFilter` ohne `@resources` — der PipeWire-Aufnahmestrom setzt beim Öffnen Echtzeitpriorität (`sched_setscheduler`); gesperrt stirbt der Dienst 120 ms nach dem Einschalten von PTT mit `status=31/SYS` (gemessen, T-5.10.v) — bei gedrücktem PTT war dAImon damit grundsätzlich taub |
