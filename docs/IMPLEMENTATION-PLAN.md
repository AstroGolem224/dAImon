...
- **Bekannte Lücken (24.08., T-6.8.v):** B5 (Ohren-Kill-Switch löscht den Hub-Automaten nicht) ist behoben (Commit `461b4c2`). Auf Anweisung zurückgestellt, nicht behoben:
  - ~~**B1/B2**~~ — **geschlossen am 26.08.**, nach einer Kette aus drei Befunden, die alle voneinander abhingen. Kurzfassung: ein Messfehler (B3) trug einen falsch geeichten Deckel, und der verdeckte einen echten Defekt. (1) Die Retention der Augen war ein **Hochwasserstand, kein Leck** — glibc hielt je einen Vollbildrahmen in drei Arenen; behoben über `MALLOC_MMAP_THRESHOLD_` und einen statt zwei OCR-Arbeitern, anon 217,6 → 92,0 MB. (2) Die Messgröße für den Speicher ist jetzt **`anon` je cgroup** statt VmRSS-Summe (die zählt geteilte Bibliotheksseiten doppelt: 149,6 gegen 92,0 MB am selben Dienst zur selben Zeit) und nicht `memory.current` (das zählt rückgewinnbaren Cache mit). (3) Beim Eichen fiel auf, dass **`OMP_NUM_THREADS=1` nie gehalten hat** — der OCR-Arbeiter zog bis 388 % eines Kerns; tesseract liest die Obergrenze aus `OMP_THREAD_LIMIT`. Mit beiden Variablen: Spitze 395,5 → 100,3 %, Bandmittel 9,49 → 3,71 %, CPU je Lauf ~2,8 → ~0,95 Kern-Sekunden bei **gleicher Arbeitsmenge** — die vier Threads hatten überwiegend im Leerlauf gedreht. Latenz je Lauf Median 0,50 s, Maximum 1,40 s, das Ziel „< 3 s" hält. Neue Deckel: **20,0 % / 110,0 % / 300 MB** — ohne den Thread-Fix wären es 80,0 % und 480,0 % geworden, und der Prüfstand hätte aufgehört, ein Wächter zu sein. `pytest -q` vollständig grün. Belege in §13.1 (Bänder B und C samt Rohproben), Commits `7599722`, `55f1eb0`, `607fe67`.
    **Ursprünglicher Stand (25.08.), zur Nachvollziehbarkeit:** notiert als „RSS 533 MB > 420 MB" und „Dauerlast 10,8 % > 2 %". Beim Schließen von B3 kam heraus, dass **die Deckel selbst auf einer defekten Eichung stehen**: §13.1 leitet sie ausdrücklich aus zwei Messbändern vom 13.08. ab (`~25 % über den 334,7 MB`, `~60 % über den 1,25 %`), und beide Bänder wurden mit dem MainPID-Sampler erhoben — also mit genau dem Instrument, dessen Untererfassung B3 war. Die gemessenen 502,1 MB (cgroup, vollständig) sind damit **kein Wachstum** gegenüber 340,4 MB, sondern ein anderes Messverfahren; die Zahlen sind nicht vergleichbar. §13.1 trägt seit dem 25.08. einen Warnkasten dazu.
    **Was zu tun ist:** die Deckel mit dem korrigierten Sampler neu ableiten, nach derselben Methode (gemessen plus Marge), über ein Band mit allen dreizehn Units. Braucht laufende Augen und ist eine Design-Entscheidung, keine Codeänderung. Bis dahin bleibt `test_ressourcen_im_budget_design_13` rot — gemessen wird richtig, verglichen gegen eine Zahl, die nicht mehr trägt.
    **Zwischenbefund für die Neueichung:** zwölf Units liegen bei 7–24 MB und summieren sich auf ~209 MB — im Wesentlichen der Grundverbrauch je Python-Prozess, rund die Hälfte des alten Deckels, bevor Fachlogik läuft. Allein `daimon-eyes` trägt 292,9 MB (3 Prozesse) gegen 108–145 MB im Design.
    **Die Augen sind nachgemessen (25.08.) — es ist gehaltene Daten, kein Bibliotheks-Sockel.** Aufteilung am laufenden Dienst: Hauptprozess 150 MB, OCR-Worker 51 MB und 95 MB (`memory.current` inkl. Cache 398 MB). Dagegen der Import-Sockel derselben Module unter demselben Interpreter: nacktes `python3` 9 MB, mit `numpy` 27 MB, mit `numpy`+`dbus`+`gi` 39 MB, voller Import von `daimon.eyes.daemon` 33 MB, `daimon.eyes.ocr_worker` 11 MB. Es bleiben also **~117 MB im Hauptprozess** und **40–84 MB je Worker** über dem Sockel. Der Hauptprozess allein überschreitet damit das Design-Budget für die gesamten Augen. Entscheidend: die beiden **identischen** Worker unterscheiden sich um 44 MB — der Verbrauch wächst mit Benutzung, ist also kein fester Preis, sondern Retention (Verdacht: Rahmenpuffer der Änderungserkennung bzw. tesseract-Allokationen je Bild).
    **Folge für die Reihenfolge:** der Deckel ist damit *nicht* offensichtlich zu eng — es gibt echte Retention zu klären. Die Neueichung sollte **danach** kommen, sonst friert sie einen Fehler als Norm ein.
    **Retention geklärt (25.08.), Messband 12 min / 48 Proben à 15 s:** es ist ein **Hochwasserstand, kein Leck**. Belegt durch Rückkehr auf denselben Boden — ein Worker fällt viermal über das Band verteilt auf exakt 70.184 KB zurück, der Daemon viermal auf 109,4 MB; ein Leck kann einen früheren Boden nicht wieder erreichen. Start und Ende des Daemons liegen 0,7 % auseinander. Positivkontrolle: 145 Runden, 15 OCR-Läufe, 10× `ocr_verschoben` — das Band ist nicht leer.
    **Ursache, am Code und am Mechanismus belegt:** 79,6 MB der 99,7 MB anonymem Speicher im Daemon sind **drei glibc-Arenen, die je genau einen Vollbildrahmen halten** (Bildschirm 5120×1440×3 = 21.600 KiB, Arenen messen 22.292/22.132 KB). Zulauf: `capture.py:89` `always-copy=true` kopiert je `quelle:src`-Faden (zwei, je Runde neu gebaut, `capture.py:147-166`), dazu `capture.py:176` `rohdaten = bytes(karte.data)` als dritte Vollkopie. glibc 2.44 hebt beim ersten Freigeben eines 21,6-MB-Blocks die **dynamische mmap-Schwelle** auf dessen Größe an — ab dann kommt der Rahmen aus der Arena und geht nie ans System zurück. Isoliert nachgestellt: ohne Schwelle +22.008 KB behalten, mit `MALLOC_MMAP_THRESHOLD_=131072` nur +408 KB.
    **Zweiter OCR-Arbeiter ist tote Last:** `ocr.py:178/183` ist LIFO auf zwei Elementen, und die Nebenläufigkeit ist 1 — `daemon.py:340` blockiert die Runde mit `zukunft.result(...)`, `daemon.py:72` lässt via `OCR_ABKUEHLUNG_S=30.0` höchstens einen Auftrag je 30 s zu. Es wird immer derselbe Arbeiter gezogen; der zweite hat über 48 Proben **Varianz null** bei 52 MB.
    **Ausgeschlossen** (die Voranalyse lag hier falsch): `change.Kette._letztes_klein` ist 160×90 Graustufen = 14,4 KB, `Fenster` ein frozen dataclass aus ints, `change.Ordner` zwei ints, `context.Kontextspeicher` durch `context.py:165` beschränkt und nur Text. Keiner wächst.
    **Wichtig für die Neueichung:** `memory.current` täuscht. Gemessen 531,7 MB, davon laut `memory.stat` nur **217,6 MB anon** und 284 MB `inactive_file` — rückgewinnbarer Seiten-Cache. Das Budget gehört gegen den anon-Anteil geführt, nicht gegen `memory.current`.
    **Nebenbefund:** `change.py:341-342` kopiert doppelt (`np.ascontiguousarray(...).copy()` — bei nicht-zusammenhängendem Schnitt liefert `ascontiguousarray` bereits eine Kopie), mit `daemon.py:338` `.tobytes()` dreifach. Transient, nicht der Haupttreiber.
    **Umgesetzt und gegengemessen (26.08.), beide Eingriffe wirken:** gleiches Band (12 min, 48 Proben à 15 s). Daemon-Mittel **132,6 → 66,6 MB (−50 %)**, Prozesse **3 → 2**, cgroup-**anon 217,6 → 92,0 MB (−58 %)**. Der Mechanismus ist direkt belegt, nicht nur die Summe: die beiden 22-MB-Arenen stehen in `smaps` auf **Rss = 0**, `[heap]` fällt von 35,0 auf 11,4 MB. `memory.current` bewegt sich kaum (531,7 → 502,6 MB), weil der Zuwachs vollständig in rückgewinnbaren Dateicache wandert (`inactive_file` 284 → 391 MB) — genau das erwartete Bild, wenn Rahmen per `mmap` statt aus der Arena kommen.
    **Positivkontrolle trägt:** 153 Runden / 24 OCR-Läufe gegen vorher 145 / 15 — das Nachher-Band trug **mehr** Last, der Rückgang ist also kein Leerlaufartefakt. Unabhängig gegengeprüft an 16 Tesseract-Läufen im Journal.
    **Falle beim Messen, für die Nachwelt:** der erste Anlauf hätte nichts gezeigt — der Dienst lief noch mit dem Code von vor der Änderung (Prozessstart 07:37, Dateizeit 08:20), (B) war im laufenden Prozess gar nicht aktiv. Ein `restart` war zwingend, und `MALLOC_MMAP_THRESHOLD_` wurde in `/proc/<pid>/environ` bestätigt statt nur in der Unit-Datei.
    **Offen: welche Zahl das Budget meint.** Die VmRSS-Summe liegt bei 149,6 MB und damit 3 % über der Obergrenze des Design-Bands; der cgroup-anon-Anteil liegt mit 92 MB deutlich darunter. VmRSS **doppelt zählt** die dateigestützten Seiten, die Daemon und Worker teilen (beide derselbe `python3` mit denselben Bibliotheken) — anon tut das nicht. Die Neueichung muss die Messgröße festlegen, sonst misst der nächste Deckel wieder etwas anderes als der vorige.
    **Vorschlag, der zu diesem Ergebnis geführt hat:** (A) `Environment=MALLOC_MMAP_THRESHOLD_=131072` in `config/systemd/daimon-eyes.service` — eine Zeile, kein Code; Kosten ein mmap je Rahmen, bei 1 Hz vernachlässigbar. (B) `ocr_arbeiter` auf 1 (`daemon.py:141`) oder ein *fauler* Pool, der den zweiten Arbeiter erst bei echtem Gleichlauf baut — ein Arbeiter für eine Nebenläufigkeit, die es nicht gibt, ist der Vorratscode aus CLAUDE.md Regel 6. Zusammen erwartet: 276 MB → grob 115 MB, wieder im Design-Band 108–145 MB. **Wirkung von (A) im Dienst ist noch nicht gemessen**, nur der glibc-Mechanismus isoliert.
  ...
  - ~~**B3** (pruefstandsfehler)~~ — **geschlossen am 25.08.** `test_ressourcen_im_budget_design_13` misst jetzt die ganze cgroup je Unit statt nur die MainPID. CPU kommt aus `cpu.stat`/`usage_usec` und nicht aus einer Summe über `/proc/<pid>/stat`: die OCR-Kinder entstehen und enden zwischen zwei Proben, eine Summe über lebende Prozesse liefe dabei rückwärts. Der Prozesssatz wird je Probe neu gelesen, sonst fiele ein erst später entstandenes Kind nie ins Messband. Dazu eine **Positivkontrolle des Messbands selbst** — bei unlesbarer cgroup meldete es 0,0 %, jedes Budget ginge durch, und der Test wäre grün, gerade weil er nichts mehr misst (genau die Falle, die B3 so lange verdeckt hat). Mechanik belegt: 31 Prozesse über die cgroup gegen 1 über MainPID, RSS 1338 MB gegen 0 MB.
    **Folge:** der Test ist jetzt rot — `idle_rss_mb` 445,7 MB > 420 MB. Das ist **B1**, das vorher unter der MainPID-Messung unsichtbar war. Auf Anweisung so stehengelassen: erst ehrlich messen, dann die Budgetfrage entscheiden.
  - ~~**B4** (pruefstandsfehler)~~ — **geschlossen am 25.08., und er wog schwerer als gemeldet.** Der Wächter verlangte `aktion.sock` **und** `connect`/`sendall` in *derselben* Zeile; das Repo hält Socketnamen aber in Konstanten (`HUB_SOCKET` in `daimon/brokers/dienst.py`), der Aufruf steht Zeilen später. Ein Fund, der von der Zeilenformatierung abhängt. Folge: `Mind.frage_werkzeug` schickt seit **T-4.16** `art: "ausfuehren"` an `aktion.sock` (`daimon/mind/daemon.py:382`), der Modulkopf des Hubs sagt das ausdrücklich, und `daimon-mind.service` steht längst auf `AKTION_UNITS` — der Wächter meldete trotzdem „kein Erzeuger". Sein Docstring versprach, an genau dem Tag aufzufallen; der Tag kam und ging unbemerkt. Gesucht wird jetzt nach der Nachrichtenart statt nach einer Schreibweise, und die Zusage lautet: **jeder Erzeuger hat seine Unit auf `AKTION_UNITS`**. Mit Positivkontrolle (kein Fund ⇒ die Prüfung wäre leer erfüllt — genau der alte Fehler) und als rot nachgewiesen gegen eine Liste ohne `daimon-mind.service`.
  - ~~**B9** (pruefstandsfehler)~~ — **geschlossen am 25.08., Befund nur zur Hälfte richtig.** Nachgemessen durch echte Startversuche: `daimon-exec` startet heute sauber (active/running, Status 0), sein Ausschlussgrund von 13.08. („Keine freigegebene Anwendung im Katalog", exit 1) ist überholt — aus `SONDERFALL_UNITS` entfernt, die Unit ist jetzt im Messband. `daimon-input` dagegen scheitert unverändert an 203/EXEC (`ProtectHome=tmpfs` verdeckt den venv-Interpreter), der Eintrag bleibt zu Recht; ebenso `daimon-egress` (243, fehlendes Token, siehe B8).
