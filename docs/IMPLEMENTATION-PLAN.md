...
- **Bekannte Lücken (24.08., T-6.8.v):** B5 (Ohren-Kill-Switch löscht den Hub-Automaten nicht) ist behoben (Commit `461b4c2`). Auf Anweisung zurückgestellt, nicht behoben:
  - **B1/B2 (offen, aber neu verstanden — 25.08.):** ursprünglich notiert als „RSS 533 MB > 420 MB" und „Dauerlast 10,8 % > 2 %". Beim Schließen von B3 kam heraus, dass **die Deckel selbst auf einer defekten Eichung stehen**: §13.1 leitet sie ausdrücklich aus zwei Messbändern vom 13.08. ab (`~25 % über den 334,7 MB`, `~60 % über den 1,25 %`), und beide Bänder wurden mit dem MainPID-Sampler erhoben — also mit genau dem Instrument, dessen Untererfassung B3 war. Die gemessenen 502,1 MB (cgroup, vollständig) sind damit **kein Wachstum** gegenüber 340,4 MB, sondern ein anderes Messverfahren; die Zahlen sind nicht vergleichbar. §13.1 trägt seit dem 25.08. einen Warnkasten dazu.
    ...
    **Zwischenbefund für die Neueichung:** zwölf Units liegen bei 7–24 MB und summieren sich auf ~209 MB — im Wesentlichen der Grundverbrauch je Python-Prozess, rund die Hälfte des alten Deckels, bevor Fachlogik läuft. Allein `daimon-eyes` trägt 292,9 MB (3 Prozesse) gegen 108–145 MB im Design. Ob das Verschwendung ist oder der Deckel zu eng, ist offen und sollte **vor** der Neueichung geklärt werden.
