...
impl App {
    ...
    fn hub_zustand_uebernehmen(&mut self, zustand: &hub::HubZustand) {
        ...
        if self.aktuelle_bubble != zustand.bubble {
    ...
    fn sprite_rendern(&mut self) {
        ...
        let callback_armieren = self.render.callback_armieren();
        ...
        match self.aktuelle_bubble.as_ref() {
            ...
            None => {
                let ausgeblendet = {
                    ...
                    overlay
                        .as_mut()
                        .is_some_and(|o| o.bubble_ausblenden(compositor))
                };
                if ausgeblendet {
                    self.diagnose_bubble_setzen(false, false);
                }
            }
        }
    }
...
impl PointerHandler for App {
    fn pointer_frame(
        ...
        let bubble_geklickt = events.iter().any(|event| {
            self.overlay
                .as_ref()
    ...
    fn configure(
        ...
        if basis_committiert {
            let sprite_ergebnis = {
                ...
                let callback_armieren = render.callback_armieren();
                overlay
                    .as_mut()
...
fn main() {
    ...
    let mut app = App {
        ...
        hub: None,
        render: RenderSteuerung::neu("sleeping", Instant::now()),
