...
fn voice_lesen(wert: &Value) -> String {
    let voice = wert.get("voice");
    let eigene_rechnung = voice.and_then(|v| {
        Some((v.get("listening")?.as_bool()?,
              v.get("tts_active")?.as_bool()?,
              v.get("denkt")?.as_bool()?))
    });
    if let Some((listening, tts_active, denkt)) = eigene_rechnung {
        return if listening { "listening" }
        else if tts_active { "speaking" }
        else if denkt { "processing" }
        else { "idle" }.to_owned();
    }
    voice
...
fn lesen_bis_ende(strom: UnixStream, sender: &Sender<HubZustand>) -> HubZustand {
    let mut letzter = HubZustand::schlafend();
    let leser = BufReader::new(strom);
    for zeile in leser.lines() {
        let Ok(zeile) = zeile else { return letzter };
