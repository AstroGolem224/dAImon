...
def vorschau(*, aktion: str, ziel, umkehr: str) -> str:
    ...
    T-3.13b: die Vorschau ist eine Senke (Design 5.2). `ziel` darf ein
    `Marked` sein — `tainted` kommt escapt und begrenzt durch, `user_audio`
    wirft `SenkenFehler`. Eine rohe Zeichenkette bleibt erlaubt (kein
    eingefrorener Pruefstand wird rot), gilt aber als `tainted` und der
    Aufruf wird protokolliert (`marke_fehlt`) — Vorgabe ist Misstrauen.
    """
    try:
    ...
    return _VORLAGE.format(aktion=aktion_text, ziel=pfad_saeubern(ziel_wert),
                           umkehr=umkehr_text)
