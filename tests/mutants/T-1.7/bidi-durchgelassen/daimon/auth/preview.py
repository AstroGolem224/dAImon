...
def _darstellbar(zeichen: str) -> bool:
    ...
    nummer = ord(zeichen)
    if 0x202A <= nummer <= 0x202E or 0x2066 <= nummer <= 0x2069:
        return True  # MUTANT: Bidi wird durchgelassen
...
def wert_saeubern(wert: str, *, max_laenge: int = MAX_WERT_LAENGE) -> str:
    ...
    normal = unicodedata.normalize("NFC", wert)
    teile = []
    ...
    # MUTANT: ASCII-Zusicherung entfernt
    return raus
