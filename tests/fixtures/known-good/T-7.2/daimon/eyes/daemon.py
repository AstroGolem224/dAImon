...
def _denylist_aus_datei() -> tuple[str, ...]:
    ...
    **Die Fensterklassen werden mitaufgenommen** (T-7.2, dritter
    Akzeptanzpunkt). Die Liste fuehrt `.desktop`-KENNUNGEN; die Kette vor dem
    Diff und der Kontextspeicher vergleichen aber die rohe `resource_class`
    von KWin. Fuer jede Anwendung, deren Klasse nicht zufaellig gleich ihrer
    Kennung ist, klafften die beiden Enden auseinander: die Redaktion sperrte
    sie (sie schlaegt in den `.desktop`-Dateien nach), die Kette liess sie
    durch -- der Bildschirm wurde gelesen, geOCRt und in den Quarantaene-
    Kontext gelegt, und nur der Archiveintrag fiel weg. "Gar nicht erfasst"
    war es damit nicht.

    Aufgeloest wird hier und nicht in den zwei Vorrichtungen: eine Liste, zwei
    Leser, eine Zuordnung. Zwei Auflösungen waeren wieder zwei Fassungen
    derselben Regel.
    """
    try:
    ...
    liste = tuple(eintraege) or DENYLIST_VORGABE
    return liste + _klassen_zu_kennungen(liste)


def _klassen_zu_kennungen(liste: Iterable[str]) -> tuple[str, ...]:
    """Jede Fensterklasse, die auf eine gelistete `.desktop`-Kennung zeigt."""
    from daimon.recorder.redaktion import desktop_kennungen

    gelistet = {s.strip().lower() for s in liste if s.strip()}
    try:
        karte = desktop_kennungen()
    except OSError:
        return ()
    return tuple(klasse for klasse, kennung in sorted(karte.items())
                 if kennung.lower() in gelistet and klasse not in gelistet)
