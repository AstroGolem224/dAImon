...
# Leerlauf endet im Prozessende (Design 5.4).
LEERLAUF_S = 300.0
...
def lauf(erkenner: Erkenner, srv: socket.socket,
         leerlauf_s: float | None = None) -> int:
    """Der Leerlauf endet im Prozessende (Design 5.4, T-7.4 Punkt 2).

    Die Frist wird bei JEDEM Aufruf aus dem Modulnamen gelesen und nicht als
    Vorgabewert eingefroren -- sonst waere sie beim Import festgelegt und
    keine Betriebsgroesse mehr.
    """
    threads: list[threading.Thread] = []
    frist = LEERLAUF_S if leerlauf_s is None else float(leerlauf_s)
