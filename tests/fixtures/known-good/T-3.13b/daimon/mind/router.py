"""Gut-Muster T-3.13b: der Router, ausschließlich aus dem Vertrag §2 gebaut.

Gegenüber T-3.13 neu — drei Stellen, alle aus der Senkentabelle:

* Eine Anfrage OHNE Marke ist eine rohe Zeichenkette an der Senke: sie gilt
  als `tainted` und der Aufruf wird protokolliert (`marke_fehlt`) — kein
  Wurf. Vorgabe ist Misstrauen, nicht Verweigerung.
* `tainted` erreicht Durchgang 1 nicht — genauso wenig wie `user_audio`.
  Wer eine Marke an die falsche Senke gibt, bekommt eine Absage, kein
  stilles Filtern.
* „Mach das" ohne benannte Aktion UND benanntes Ziel ist eine
  Handlungs-Anapher und führt zu einer Rückfrage, nicht zu einer Auflösung
  aus dem Kontext.

Keine Zeile hier ist aus der echten Implementierung übernommen — der
Reviewer hat sie nie gesehen.
"""
...
from daimon.mind.answer import DurchgangZwei, lade_persona_prompt
...
# Worte, die ein benanntes Ziel markieren. „mach das fenster zu" nennt
# eines, „mach das" nicht — nur Letzteres ist eine Handlungs-Anapher.
MUSTER_ZIEL = ["fenster", "window", "datei", "prozess", "lautstärke",
               "lautstaerke", "w_"] + APP_IDS

# Die vier Marken des Vertrags. Alles andere — fehlend, falsch getippt —
# ist fehlendes Wissen und wird konservativ als `tainted` behandelt.
MARKEN = frozenset({"user_ptt", "user_audio", "trusted", "tainted"})

ABLEHNUNG_AKTION = "Das kann ich noch nicht — es fehlt der Ausführer."
RUECKFRAGE_AKTION = "Was soll ich womit machen?"

_log = get_logger("daimon-mind")
...
def hat_ziel(text: str) -> bool:
    t = text.lower()
    return any(m in t for m in MUSTER_ZIEL)
...
class Router:
    def __init__(self) -> None:
        ...
        self.runden = 0
        self.api_aufrufe = 0
        self._zwei = DurchgangZwei(self._laufzeit, lade_persona_prompt(),
                                   self._inhalt)
    ...
    def frage(self, req: dict) -> dict:
        ...
        runde = req.get("runde")
        # Die Marke: was fehlt oder unbekannt ist, ist eine rohe
        # Zeichenkette an der Senke — sie gilt als `tainted`, und der
        # Aufruf wird protokolliert. Kein Wurf: Vorgabe ist Misstrauen,
        # nicht Verweigerung.
        marke = req.get("marke")
        if marke not in MARKEN:
            senke = "durchgang2" if absicht == "api" else "durchgang1"
            _log.warn("Anfrage ohne gueltige Marke",
                      DAIMON_ACTION="marke_fehlt", DAIMON_SENKE=senke)
            marke = "tainted"
        # Senkentabelle (Design §5.2): weder `user_audio` noch `tainted`
        # erreicht Durchgang 1. Die Senke steht vor jeder Quellenabfrage
        # und vor jedem Ticketversuch — es wird abgesagt, nicht still
        # gefiltert.
        ...
        try:
            if absicht == "aktion":
                if not hat_ziel(text):
                    # Handlungs-Anapher: keine Aktion, kein API-Aufruf,
                    # sondern eine Rückfrage (Antwortform des Vertrags).
            ...
            if absicht == "fensterliste":
                ...
                # Ein Fenstertitel ist angreiferbeeinflusst: tainted. Die
                # Markierung folgt der Herkunft der Daten, nicht der
